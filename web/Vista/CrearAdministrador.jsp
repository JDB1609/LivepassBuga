...
<%
  ...
  String id       = (String) request.getAttribute("f_id");
  ...
  <main class="relative">
    <section class="max-w-6xl mx-auto px-5 min-h-[calc(100vh-90px)] grid md:grid-cols-2 gap-12 items-center">
      ...
      <div class="animate-fadeRight">
        <p class="text-aqua font-semibold mb-2">Panel administrativo</p>
        <h1 class="text-4xl md:text-5xl font-extrabold leading-tight">
          Crear nuevo <span class="text-gradient-anim">Administrador</span>
        </h1>
        <p class="text-white/70 mt-3 max-w-xl">Solo los administradores autorizados pueden registrar nuevos usuarios con permisos de administración.</p>
        <nav class="mt-6 text-white/70">
          <a href="BackofficeAdministrador.jsp" class="hover:text-white transition">Inicio</a>
          <span class="mx-2 opacity-50">/</span><span class="opacity-90">Crear Administrador</span>
        </nav>
      </div>
      ...
      <div class="animate-fadeUp">
        <div class="w-full max-w-md glass ring rounded-2xl p-6 mx-auto js-tilt">
          ...
          <form action="<%= request.getContextPath() %>/Control/ct_crear_administrador.jsp" method="post" class="space-y-4" autocomplete="off">
            
            <div>
              <label class="block text-sm text-white/80 mb-1">Cédula</label>
              <input name="id" value="<%= id!=null?id:"" %>" required
                     class="w-full px-4 py-3 rounded-xl bg-white/5 ring focus:outline-none focus:ring-2 focus:ring-primary"
                     placeholder="Ej: 1012345678" />
              <% if (errors!=null && errors.get("id")!=null){ %>
                <div class="text-pink-300 text-sm mt-1"><%= errors.get("id") %></div>
              <% } %>
            </div>
            ...
            <button class="w-full btn-primary ripple" type="submit">Crear administrador</button>
          </form>
